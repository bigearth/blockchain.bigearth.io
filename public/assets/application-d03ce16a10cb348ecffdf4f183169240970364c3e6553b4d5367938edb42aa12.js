...
(function() {
  $(function() {
    ...
    Panel = (function() {
      ...
      Panel.prototype.update = function(action_type) {
        $('#title_text').text(action_type);
        $('#details').text(this.panel_data[action_type].details);
        $('#http_verb').removeClass('get post put delete').addClass(this.panel_data[action_type].verb).text(this.panel_data[action_type].verb);
        return $('#action input').val(action_type);
      };
      ...
      Blockchain.prototype.bind_events = function() {
        ...
        $('#controls ul a:not(#destroy_chain)').click((function(_this) {
          return function(evt) {
            ...
            _this.panel.update($action);
            return evt.preventDefault();
          };
        })(this));
        return $('#send').click((function(_this) {
          return function(evt) {
            var $action, blockchain_property, href;
            ...
            $.get(href, {
              ipv4_address: $('#controls').data('ipv4_address')
            }, function(rsp) {
              _this.clear_output('complete');
              _this.clear_output('in_progress');
        ...
        return $.get("confirm_node_created?title=" + ($('#blockchain_title').data('title')), (function(_this) {
          return function(rsp) {
            if (rsp.message === 'node created') {
              ...
              _this.blockchain.update_output($("<li>IPv4 Address: " + rsp.ipv4_address + ".</li>"), 'complete');
              _this.blockchain.update_output($("<li>IPv6 Address: " + rsp.ipv6_address + ".</li>"), 'complete');
