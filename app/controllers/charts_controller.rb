class ChartsController < ApplicationController
  ...
  def circulation
    @circulations = Charts::Circulation.all.map do |item| 
      [DateTime.strptime(item['time'].to_s,'%s').strftime("%b'%y"), item['total'].to_i]
    end 
  end
  ...
  # GET /charts/total_output_volume
  # GET /charts/total_output_volume.json
  def total_output_volume
    @total_output_volume = Charts::TotalOutput.all.map do |item| 
      [DateTime.strptime(item['time'].to_s,'%s').strftime("%b'%y"), item['total'].to_i]
    end 
  end
